...
export async function injectChatReplyViaCDP(
  ...
  const injectScript = \`(function() {
    ...
    var findSendButton = function() {
      var selectors = [
        '.b-chat__btn-submit', 'button.b-btn-send-message', 'button[data-testid*="send"]',
        'button[aria-label*="Send"]', 'button[aria-label*="send"]',
        '.b-chat-message-input button[type="submit"]', '.b-chat__input button[type="submit"]',
        'form button[type="submit"]', 'button[class*="send"]', 'button[class*="submit"]',
        '[aria-label*="paper plane"]', '[title*="Send"]'
      ];
      for (var i = 0; i < selectors.length; i++) {
    ...
    if (sendBtn) {
      clickButton(sendBtn);
      result.success = true; result.autoSent = true; result.sendStrategy = 'button_click';
      return JSON.stringify(result);
    }

    var form = input.closest ? input.closest('form') : null;
    if (form && typeof form.requestSubmit === 'function') {
      try { form.requestSubmit(); result.success = true; result.autoSent = true; result.sendStrategy = 'form_submit'; return JSON.stringify(result); } catch (_) {}
    }
    ...
    result.success = true; result.autoSent = false; result.sendStrategy = 'enter_fallback';
