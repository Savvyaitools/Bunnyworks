...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create_admin_session") {
      ...
      const sess = await bb(BK, "/sessions", { method: "POST", body: JSON.stringify({ projectId: BP, browserSettings: { context: { id: ctxId, persist: true }, fingerprint: { browsers: ["chrome"], operatingSystems: ["windows"] } }, proxies: proxyConf(cr), keepAlive: false, timeout: 900, userMetadata: { creatorId, agencyId, userId: uid, platform, sessionType: "admin", creatorName: cr?.name || "Unknown" } }) });
    ...
    if (action === "launch_chatter_session") {
      ...
      const cfg: any = { projectId: BP, browserSettings: { context: { id: link.browserbase_context_id, persist: true }, fingerprint: { browsers: ["chrome"], operatingSystems: ["windows"] } }, proxies: proxyConf(cr), keepAlive: false, timeout: 3600, userMetadata: { creatorId: link.creator_id, agencyId: link.agency_id, chatterId: chatterId || uid, platform: link.platform, sessionType: "chatter", creatorName: cr?.name || "Unknown", permissions: permFlags } };
